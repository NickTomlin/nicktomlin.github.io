...
function Nav (props) {
  ...
  return (
    <nav {...props} className={"overflow-x-hidden overflow-hidden"}>
      <span onClick={handleToggle} className={"block w-10 h-6 cursor-pointer p-5 transition-color"}>
        <svg>
      ...
      <div className={clsx(!visible  && "hidden")} >
        <div className={"mt-4 absolute border-1 border-light-blue-500 flex flex-col space-y-3"} onClick={handleClick}>
          <Link href={"/"} >Home</Link>
...
export function Header () {
  return (
    <header className="relative my-3 min-h-56">
